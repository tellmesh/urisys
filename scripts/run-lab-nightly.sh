...
# Requires tellmesh monorepo checkout (urisys + uricore + uri3 + uri2flow siblings).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

if [ ! -d "../uricore" ] && [ ! -d "uricore" ]; then
  echo "error: uricore sibling required (tellmesh monorepo layout)" >&2
  exit 1
fi
