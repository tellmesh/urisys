...
class MarkpactManager:
    ...
    def compile(self, path: str | Path, *, out_dir: str | Path | None = None, force: bool = False) -> CompiledMarkpact:
        ...
        self._write_handler_modules(package_dir, self._handler_blocks(blocks))
    ...
    def run_tests(self, path: str | Path, *, events_path: str | Path | None = None) -> dict[str, Any]:
        ...
        try:
            for item in tests:
                ...
                ok, failures = self._check_expectations(result, item.get("expect") or {})
    ...
    @staticmethod
    def _check_expectations(result: dict[str, Any], expect: dict[str, Any]) -> tuple[bool, list[str]]:
