...
export PYTHONPATH="$ROOT/packages/python"
...
python -m urisysedge --device-config "$ROOT/config/device-profile.json" --events "$ROOT/data/events.jsonl" flow "$ROOT/flows/move-test.uri.flow.yaml" --approve --dry-run
